    ...
    for (size_t i = 0; i < g_runtime.program_count; i++) {
        ...
        if (program) {
            ...
            for (int j = 0; j < NET_MAX_CLIENTS; j++) {
                NET_MUTEX_LOCK(&program->clients[j].lock);
                if (program->clients[j].is_active) {
                    printf("  Client %d: Connected\n", j);
                }
                NET_MUTEX_UNLOCK(&program->clients[j].lock);
            }
...
static bool add_port_mapping(const char* service_name, uint16_t host_port, uint16_t container_port) {
    ...
    if (stat(path_buf, &st) == -1 && mkdir(path_buf
#ifndef _WIN32
        , 0755
#endif
        ) == -1) {
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            printf("PolyCall CLI v%s\n\n", PPI_VERSION);
            printf("Usage: polycall [options]\n\n");
            printf("Options:\n");
            printf("  -f <file>    Load configuration file (non-interactive mode)\n");
            printf("  --help       Show this help message\n");
            printf("  --version    Show version information\n");
            printf("\nInteractive commands: type 'help' once running.\n");
            return 0;
...
#ifndef _WIN32
            fprintf(stderr, "Error: --daemon mode not yet implemented; use -f <config> for non-interactive mode.\n");
#else
            fprintf(stderr, "Error: --daemon is not supported on Windows.\n");
#endif
            return 1;
